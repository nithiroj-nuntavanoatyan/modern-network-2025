...
# # * `s3`: AS 500 LAN (connects h3 and r5)
# # * `link_45`: Link between r4 and r5
...
sudo docker run -itd --net=none --name h3 --privileged alpine:latest sh
...
sudo docker run -d --net=none --name r5 --privileged my-alpine-router
...
#### **3. Connect AS 500 (Campus-3)**

# * **h3 (eth0) <-> s3**
# * **r5 (eth0) <-> s3**

# Connect h3 to s3 (IP: 192.168.3.10/24)
sudo ovs-docker add-port s3 eth0 h3 --ipaddress=192.168.3.10/24
sudo docker exec h3 route add default gw 192.168.3.1

# Connect r5 to s3 (IP: 192.168.3.1/24)
sudo ovs-docker add-port s3 eth0 r5 --ipaddress=192.168.3.1/24

#Check
sudo docker exec -it h3 sh -c "ifconfig && route && ping 192.168.3.1 -c 5"

#### **4. Connect ISP Links (AS 300 & Inter-AS)**

# * **r1 (eth1) <-> r3 (eth0)** via `link_13`
# * **r3 (eth1) <-> r4 (eth0)** via `link_34`
# * **r4 (eth1) <-> r2 (eth1)** via `link_24`
# * **r4 (eth2) <-> r5 (eth1)** via `link_45`
...
# Link r4 -> r5
sudo ovs-docker add-port link_45 eth2 r4 --ipaddress=192.168.45.2/30
sudo ovs-docker add-port link_45 eth1 r5 --ipaddress=192.168.45.1/30
sudo docker exec -it r4 sh -c "ping 192.168.45.1 -c 5"
...
network 192.168.45.0/30 area 0
end
exit
...
neighbor 192.168.45.1 remote-as 500
end
exit
...
# Access Router r5 (AS 500):
sudo docker exec -it r5 vtysh
configure terminal
router bgp 500
no bgp ebgp-requires-policy
neighbor 192.168.45.2 remote-as 300
...
sudo docker exec -it h1 ping -c 3 192.168.2.10
sudo docker exec -it h1 ping -c 3 192.168.3.10
sudo docker exec -it h2 ping -c 3 192.168.3.10
sudo docker exec -it h3 ping -c 3 192.168.1.10
